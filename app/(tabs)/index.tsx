...
import {
  VeiculoService,
  type Veiculo,
} from "../../src/services/veiculoService";
...
export default function TelaBomba() {
  ...
  const [combustivelAbastecido, setCombustivelAbastecido] = useState<
    "ETANOL" | "GASOLINA"
  >("GASOLINA");
  ...
  const [ultimoKmRegistrado, setUltimoKmRegistrado] = useState<number | null>(
    null,
  );
  ...
  const atualizarMediasPorVeiculo = (carroId: string) => {
    ...
    const consumoOtimizado =
      AbastecimentoService.calcularConsumoOtimizadoPorIA(idVeiculo);
  ...
  const analisarCombustivel = () => {
    if (!precoEtanol || !precoGasolina) {
      Alert.alert(
        "Atenção",
        "Insira os preços de ambos os combustíveis para analisar.",
      );
      return;
    }
  ...
  const salvarAbastecimento = () => {
    if (!veiculoSelecionadoId || !odometroAtual || !litrosAbastecidos) {
      Alert.alert(
        "Atenção",
        "Preencha todos os campos do abastecimento corrente.",
      );
      return;
    }
    ...
    const preco = parseFloat(
      combustivelAbastecido === "ETANOL" ? precoEtanol : precoGasolina,
    );

    if (!preco) {
      Alert.alert(
        "Erro",
        "Digite o preço do combustível escolhido no bloco superior.",
      );
      return;
    }

    // Calcula a distância percorrida
    const resultDistancia = AbastecimentoService.calcularDistancia(
      idVeiculo,
      kmAtual,
    );
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.secao}>
        ...
        <View style={styles.pickerContainer}>
          {veiculos.length > 0 ? (
            ...
            <Text style={styles.avisoNenhumCarro}>
              Nenhum carro na garagem. Vá na aba veículos!
            </Text>
          )}
        </View>
      </View>
      ...
      <View
        style={[styles.cardFeedback, usandoIA ? styles.bgIA : styles.bgNormal]}
      >
        <Text style={styles.tituloFeedback}>
          {usandoIA
            ? "🤖 Consumo Otimizado por Regressão Linear"
            : "📊 Consumo Estimado (Sem histórico de IA)"}
        </Text>
        <View style={styles.rowInputs}>
          <View style={{ width: "48%" }}>
      ...
      {resultadoDecisao ? (
        <View
          style={[
            ...
            usandoIA ? styles.bordaIA : styles.bordaNormal,
          ]}
        >
      ...
      <View style={styles.secaoAbastecimento}>
        <Text style={styles.subtitulo}>
          Confirmou o abastecimento? Registre:
        </Text>
        ...
        <View style={styles.rowBotoesCombustivel}>
          <TouchableOpacity
            style={[
              ...
              combustivelAbastecido === "ETANOL" && styles.ativoEtanol,
            ]}
          ...
          <TouchableOpacity
            style={[
              styles.botaoCombustivel,
              combustivelAbastecido === "GASOLINA" && styles.ativoGasolina,
            ]}
        ...
        <View style={styles.linhaUltimoKm}>
          ...
          <Text style={styles.valorUltimoKm}>
            {ultimoKmRegistrado !== null
              ? `${ultimoKmRegistrado} km`
              : "Nenhum registro anterior"}
          </Text>
        </View>

        <View>
          <Text style={styles.miniLabel}>km atual do painel:</Text>
          <TextInput
            placeholder="KM atual do painel"
            keyboardType="numeric"
            value={odometroAtual}
            onChangeText={setOdometroAtual}
            style={[styles.input, { marginBottom: 12 }]}
          />
        </View>
        <View >
          <Text style={styles.miniLabel}>Quantos litros colocou?</Text>
          <TextInput
            placeholder="Quantos litros colocou?"
            keyboardType="numeric"
        ...
        <View style={styles.rowSwitch}>
          <Text style={styles.rotuloSwitch}>
            Completou o tanque (Tanque Cheio)?
          </Text>
          <Switch
            value={isTanqueCheio}
            onValueChange={setIsTanqueCheio}
            trackColor={{ false: "#767577", true: "#34c759" }}
          />
        </View>
        <Text style={styles.legendaSwitch}>
          *Abastecimentos parciais não serão usados para treinar o modelo
          preditivo da IA, mantendo os dados precisos.
        </Text>
